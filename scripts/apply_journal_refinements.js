              ...
              <motion.div variants={fade} className="bg-white rounded-[28px] border border-slate-200/80 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.32)] overflow-hidden">
                ...
                <div className="divide-y divide-slate-100/80">
                  {pageErrors.map((err, i) => {
                    ...
                    return (
                      <div key={i} className="px-6 sm:px-8 py-6 sm:py-7 space-y-6 hover:bg-slate-50/30 transition-colors">
                        ...
                        <div className="rounded-2xl bg-slate-50/70 border border-slate-200/80 p-5 md:p-6 shadow-2xs">
                          ...
                          <MathRenderer
                            ...
                            block
                            className="text-[16px] sm:text-[17px] text-slate-900 leading-relaxed font-semibold whitespace-pre-wrap break-words"
                          />
                        </div>
                          ...
                          {hasMcqOptions && (() => {
                            ...
                            return (
                              <div className="space-y-3">
                                <div className={\`grid gap-3 sm:gap-4 \${hasLongOptions ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2"}\`}>
                                  {uniqueOptions.map((item, oi) => {
                                    ...
                                    return (
                                      <div key={oi} className={\`flex items-center gap-4 p-4 rounded-xl border transition-all \${
                                        isCorrect
                                        ...
                                        <span className={\`w-7 h-7 min-w-[28px] min-h-[28px] rounded-lg text-xs font-black flex items-center justify-center flex-shrink-0 \${
                                          isCorrect ? "bg-emerald-600 text-white shadow-2xs" :
                                          isStudentAnswer ? "bg-rose-500 text-white shadow-2xs" :
                                          "bg-slate-100 text-slate-500"
                                        }\`}>{String.fromCharCode(65 + oi)}</span>
                                        <div className="flex-1 min-w-0">
                                          <MathRenderer
                                            text={item.latex}
                                            className={\`text-[14px] sm:text-[14.5px] leading-snug break-words \${
                                              isCorrect ? "font-semibold text-emerald-950" :
                                              isStudentAnswer && !isCorrect ? "font-semibold text-rose-950" :
                                ...
                                {!hasMatchedStudentOpt && err.student_answer && !["idk", "__wrong__", "test"].includes(err.student_answer.trim().toLowerCase()) && (
                                  <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-rose-50/70 border border-rose-200/80 text-xs font-medium text-rose-950">
                                    <XCircle className="w-4 h-4 text-rose-500 flex-shrink-0" />
                                    <span className="font-semibold text-rose-700">Введённый ответ ученика:</span>
                                    <MathRenderer text={err.student_answer_latex || err.student_answer} className="font-semibold text-[14px]" />
                                  </div>
                                )}
                              </div>
                            );
                          ...
                          {!hasMcqOptions && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Student answer */}
                              <div className="rounded-xl border bg-white border-rose-200/90 p-4 sm:p-5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2">
                                  <XCircle className="w-4 h-4 text-rose-400" />
                                  <span className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-slate-400">Ответ ученика</span>
                                </div>
                                {err.student_answer
                                  ? <MathRenderer text={err.student_answer_latex || err.student_answer} block className="text-[14px] sm:text-[14.5px] font-semibold text-rose-950 leading-relaxed break-words" />
                                  : <span className="text-xs font-semibold text-slate-400">—</span>
                                }
                              </div>
                              {/* Correct answer */}
                              <div className="rounded-xl border bg-emerald-50/70 border-emerald-200/90 p-4 sm:p-5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2">
                                  <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                                  <span className="text-[10.5px] font-bold uppercase tracking-[0.18em] text-slate-400">Верный ответ</span>
                                </div>
                                {(err.correct_answer_latex || err.correct_answer)
                                  ? <MathRenderer text={err.correct_answer_latex || err.correct_answer || ""} block className="text-[14px] sm:text-[14.5px] font-semibold text-emerald-950 leading-relaxed break-words" />
                                  : <span className="text-xs font-semibold text-slate-400">—</span>
                                }
                              </div>
                            </div>
                          ...
                          {isDistractor && err.distractor_explanation && (
                            <div className="rounded-2xl bg-indigo-50/40 border border-indigo-100/90 p-5 md:p-6 shadow-2xs relative">
                              <div className="mb-3 flex items-center gap-2 text-[10.5px] font-bold uppercase tracking-[0.18em] text-indigo-600">
                                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 shrink-0 inline-block" />
                                Анализ ошибки
                              </div>
                              <div className="text-[14px] sm:text-[14.5px] text-slate-700 leading-relaxed font-normal">
                                <MathRenderer text={err.distractor_explanation_latex || err.distractor_explanation} className="text-[14px] sm:text-[14.5px] text-slate-700 leading-relaxed" />
                              </div>
                            </div>
                          )}
                          ...
                          {isDistractor && !err.distractor_explanation && (
                            <div className="rounded-2xl bg-indigo-50/30 border border-indigo-100/80 p-4 sm:p-5 shadow-2xs">
                              <div className="mb-2.5 flex items-center gap-2 text-[10.5px] font-bold uppercase tracking-[0.18em] text-indigo-600">
                                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 shrink-0 inline-block" />
                                Анализ ошибки
                              </div>
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log("Successfully updated journal styles in page.tsx!");
} else {
