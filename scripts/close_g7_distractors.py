#!/usr/bin/env python3
"""Close all G7 distractor gaps — numeric + text, loop until done."""
...
log = logging.getLogger("close_g7_distractors")
...
FETCH_SQL = """
    ...
    WHERE tb.class_level = 7
      AND COALESCE(tm.correct_answer, '') != ''
...
def run_pass(engine, *, limit: int, sleep: float) -> dict[str, int]:
    ...
    with engine.connect() as conn:
        rows = conn.execute(text(FETCH_SQL), {"limit": limit}).fetchall()
...
def main() -> int:
    ...
    log.info("Remaining gaps: %s | exhausted: %s", gaps, exhausted)
    return 0
