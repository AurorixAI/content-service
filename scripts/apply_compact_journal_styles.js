              ...
              <motion.div variants={fade} className="bg-white rounded-[28px] border border-slate-200/80 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.32)] overflow-hidden">
                ...
                <div className="divide-y divide-slate-100/80">
                  {pageErrors.map((err, i) => {
                    ...
                    return (
                      <div key={i} className="px-6 py-5 space-y-3.5 hover:bg-slate-50/30 transition-colors">

                        {/* ── Entry Top Bar: number pill + skill name ── */}
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="px-2 py-0.5 rounded-md text-[10.5px] font-black tracking-wider bg-slate-900 text-white shadow-2xs">
                            № {globalIdx}
                          </span>
                          {err.skill_name_ru && (
                        ...
                        <div className="rounded-2xl bg-slate-50/70 border border-slate-200/80 p-4 sm:p-4.5 shadow-2xs">
                          <div className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
                          ...
                          {hasMcqOptions && (() => {
                            ...
                            return (
                              <div className="space-y-2.5">
                                <div className={\`grid gap-2.5 sm:gap-3 \${hasLongOptions ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2"}\`}>
                                  {uniqueOptions.map((item, oi) => {
                                    ...
                                    return (
                                      <div key={oi} className={\`flex items-center gap-3 p-3 sm:p-3.5 rounded-xl border transition-all \${
                                        isCorrect
                                ...
                                {!hasMatchedStudentOpt && err.student_answer && !["idk", "__wrong__", "test"].includes(err.student_answer.trim().toLowerCase()) && (
                                  <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-rose-50/70 border border-rose-200/80 text-xs font-medium text-rose-950">
                          ...
                          {!hasMcqOptions && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {/* Student answer */}
                              <div className="rounded-xl border bg-white border-rose-200/90 p-3.5 sm:p-4 shadow-2xs">
                              ...
                              {/* Correct answer */}
                              <div className="rounded-xl border bg-emerald-50/70 border-emerald-200/90 p-3.5 sm:p-4 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-1.5">
                          ...
                          {isDistractor && err.distractor_explanation && (
                            <div className="rounded-2xl bg-indigo-50/40 border border-indigo-100/90 p-4 sm:p-4.5 shadow-2xs relative">
                          ...
                          {isDistractor && !err.distractor_explanation && (
                            <div className="rounded-2xl bg-indigo-50/30 border border-indigo-100/80 p-3.5 sm:p-4 shadow-2xs">
                              <div className="mb-1.5 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-indigo-500/90">
                                <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                                Анализ ошибки
                              </div>
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log("Successfully updated page.tsx with compact, matching styles!");
} else {
