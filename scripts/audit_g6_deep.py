#!/usr/bin/env python3
"""G6 deep quality audit — tasks, verify, distractors, gate, LaTeX."""
...
def main() -> int:
    ...
    ap.add_argument("--export-gate-fails", type=Path, help="Write gate-fail IDs JSON")
    args = ap.parse_args()
            ...
            SELECT
              ...
              count(*) FILTER (
                WHERE jsonb_array_length(COALESCE(distractor_meta,'[]')) >= 2
                  AND EXISTS (
                    SELECT 1 FROM jsonb_array_elements(distractor_meta) d
                    WHERE coalesce(d->>'error_logic','') = ''
                      AND coalesce(d->>'explanation','') != ''
                  )
              ) AS legacy_expl_only
            FROM tasks_master tm
            JOIN textbook_toc toc ON toc.id = tm.toc_id
    ...
    for i, row in enumerate(dist_rows):
        ...
        fail_list.append((row["id"], at, reasons, a[:50]))
    ...
    for tid, at, reasons, ans in show:
        print(f"    {tid}  [{at}]  {reasons}  ans={ans}")
    if len(fail_list) > 60:
        print(f"    ... +{len(fail_list) - 60} more")

    if args.export_gate_fails and export_payload:
        args.export_gate_fails.write_text(
            json.dumps(export_payload, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(f"\n  Exported {len(export_payload)} gate-fail IDs -> {args.export_gate_fails}")

    print("\n## 7. ПЕДАГОГИКА ДИСТРАКТОРОВ (SQL)")
    print(f"  short_error_logic (<10 симв.): {ped['short_error_logic']}")
    print(f"  missing_error_logic (оба пусты): {ped['missing_error_logic']}")
    print(f"  legacy_expl_only (только explanation): {ped['legacy_expl_only']}")
