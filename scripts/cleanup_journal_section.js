              ...
              <motion.div variants={fade} className="bg-white rounded-[28px] border border-slate-200/80 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.32)] overflow-hidden">
                ...
                <div className="divide-y divide-slate-100/80">
                  {pageErrors.map((err, i) => {
                    ...
                    return (
                      <div key={i} className="px-6 py-6 space-y-4 hover:bg-slate-50/30 transition-colors">
                        ...
                        <div className="rounded-2xl bg-slate-50/70 border border-slate-200/80 p-4.5 sm:p-5 shadow-2xs">
                          <div className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">
                          ...
                          <MathRenderer
                            ...
                            block
                            className="text-[14.5px] sm:text-[15px] text-slate-800 leading-relaxed font-medium whitespace-pre-wrap break-words"
                          />
                        </div>
                              ...
                              <div className="space-y-3">
                                    ...
                                    return (
                                      <div key={oi} className={\`flex items-center gap-3 p-3.5 sm:p-4 rounded-xl border transition-all \${
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <MathRenderer
                                            text={item.latex}
                                            className={\`text-[14px] sm:text-[14.5px] leading-relaxed break-words \${
                                        ...
                                        {isStudentAnswer && !isCorrect && <XCircle className="w-4 h-4 text-rose-400 flex-shrink-0" />}
                                      </div>
                                    );
                                  })}
                                </div>
                                {!hasMatchedStudentOpt && err.student_answer && !["idk", "__wrong__", "test"].includes(err.student_answer.trim().toLowerCase()) && (
                                  <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-rose-50/70 border border-rose-200/80 text-xs font-medium text-rose-950">
                                    ...
                                    <MathRenderer text={err.student_answer_latex || err.student_answer} className="font-medium text-[14px]" />
                                  </div>
                                )}
                              </div>
                            );
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              ...
                              <div className="rounded-xl border bg-white border-rose-200/90 p-4 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2">
                                  ...
                                  <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Ответ ученика</span>
                                </div>
                                {err.student_answer
                                  ? <MathRenderer text={err.student_answer_latex || err.student_answer} block className="text-[14px] sm:text-[14.5px] font-medium text-rose-950 leading-relaxed break-words" />
                              ...
                              <div className="rounded-xl border bg-emerald-50/70 border-emerald-200/90 p-4 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2">
                                  ...
                                  <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-400">Верный ответ</span>
                                </div>
                                {(err.correct_answer_latex || err.correct_answer)
                                  ? <MathRenderer text={err.correct_answer_latex || err.correct_answer || ""} block className="text-[14px] sm:text-[14.5px] font-medium text-emerald-950 leading-relaxed break-words" />
                                  : <span className="text-sm font-semibold text-slate-400">—</span>
                                }
                              </div>
                            </div>
                          ...
                          {isDistractor && err.distractor_explanation && (
                            <div className="rounded-2xl bg-gradient-to-br from-indigo-50/70 via-white to-indigo-50/30 border border-indigo-200/80 p-4.5 sm:p-5 shadow-2xs relative">
                              <div className="mb-2.5">
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-indigo-100/90 text-indigo-900 text-[10.5px] font-bold tracking-wider uppercase">
                                  Анализ ошибки
                                </span>
                              </div>
                              <div className="text-[14px] sm:text-[14.5px] text-slate-800 leading-relaxed font-normal">
                                <MathRenderer text={err.distractor_explanation_latex || err.distractor_explanation} className="text-[14px] sm:text-[14.5px] text-slate-800 leading-relaxed" />
                              </div>
                            </div>
                          )}
                          ...
                          {isDistractor && !err.distractor_explanation && (
                            <div className="rounded-xl bg-indigo-50/40 border border-indigo-100/80 p-4 shadow-2xs">
                              <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-indigo-100/70 text-indigo-900 text-[10.5px] font-bold tracking-wider uppercase">
                                Анализ ошибки
                              </span>
                              <p className="text-xs font-medium text-slate-700 mt-1.5">Ученик выбрал характерный ложный вариант ответа (дистрактор).</p>
                            </div>
                          )}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log("Successfully cleaned up Journal section in page.tsx!");
} else {
