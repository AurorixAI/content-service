          ...
          {activeTab === "journal" && rj.error_patterns.length > 0 && (() => {
            ...
            return (
              <motion.div variants={fade} className="bg-white rounded-[28px] border border-slate-200/80 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.32)] overflow-hidden">
                ...
                {/* ── Journal Header ── */}
                <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-slate-50/70">
                ...
                <div className="divide-y divide-slate-100/80">
                  {pageErrors.map((err, i) => {
                    ...
                    return (
                      <div key={i} className="px-6 py-6 space-y-5 hover:bg-slate-50/30 transition-colors">

                        {/* ── Entry Top Bar: number pill + skill name ── */}
                        <div className="flex items-center gap-2.5 flex-wrap">
                          <span className="px-2.5 py-0.5 rounded-lg text-[11px] font-black tracking-wider bg-slate-900 text-white shadow-2xs">
                            № {globalIdx}
                          </span>
                          {err.skill_name_ru && (
                        ...
                        <div className="rounded-2xl bg-slate-50/70 border border-slate-200/80 p-5 md:p-6 shadow-2xs">
                          <div className="mb-2.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">
                          ...
                          <MathRenderer
                            ...
                            block
                            className="text-[14.5px] md:text-[15px] text-slate-850 leading-[1.75] font-medium whitespace-pre-wrap break-words"
                          />
                        </div>
                          ...
                          {hasMcqOptions && (() => {
                            ...
                            return (
                              <div className="space-y-3">
                                <div className={\`grid gap-4 \${hasLongOptions ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2"}\`}>
                                  {uniqueOptions.map((item, oi) => {
                                    ...
                                    return (
                                      <div key={oi} className={\`flex items-center gap-3.5 p-4 md:p-5 rounded-2xl border transition-all \${
                                        isCorrect
                                        ...
                                        <div className="flex-1 min-w-0">
                                          <MathRenderer
                                            text={item.latex}
                                            className={\`text-[14.5px] md:text-[15px] leading-relaxed break-words \${
                                        ...
                                        {isStudentAnswer && !isCorrect && <XCircle className="w-5 h-5 text-rose-500 flex-shrink-0" />}
                                      </div>
                                    );
                                  })}
                                </div>
                                {!hasMatchedStudentOpt && err.student_answer && !["idk", "__wrong__", "test"].includes(err.student_answer.trim().toLowerCase()) && (
                                  <div className="flex items-center gap-2.5 px-4.5 py-3 rounded-xl bg-rose-50/70 border border-rose-200/80 text-sm font-medium text-rose-950">
                                    <XCircle className="w-4 h-4 text-rose-500 flex-shrink-0" />
                                    <span className="font-semibold text-rose-700">Введённый ответ ученика:</span>
                                    <MathRenderer text={err.student_answer_latex || err.student_answer} className="font-medium text-[14.5px]" />
                                  </div>
                                )}
                              </div>
                            );
                          ...
                          {!hasMcqOptions && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Student answer */}
                              <div className="rounded-2xl border bg-white border-rose-200/90 p-5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2.5">
                                  <XCircle className="w-4 h-4 text-rose-400" />
                                  <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Ответ ученика</span>
                                </div>
                                {err.student_answer
                                  ? <MathRenderer text={err.student_answer_latex || err.student_answer} block className="text-[14.5px] md:text-[15px] font-medium text-rose-950 leading-[1.75] break-words" />
                                  : <span className="text-sm font-semibold text-slate-400">—</span>
                                }
                              </div>
                              {/* Correct answer */}
                              <div className="rounded-2xl border bg-emerald-50/70 border-emerald-200/90 p-5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2.5">
                                ...
                                {(err.correct_answer_latex || err.correct_answer)
                                  ? <MathRenderer text={err.correct_answer_latex || err.correct_answer || ""} block className="text-[14.5px] md:text-[15px] font-medium text-emerald-950 leading-[1.75] break-words" />
                                  : <span className="text-sm font-semibold text-slate-400">—</span>
                                }
                              </div>
                            </div>
                          ...
                          {isDistractor && err.distractor_explanation && (
                            <div className="rounded-2xl bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-blue-50/40 border border-blue-200/90 p-5 md:p-6 shadow-2xs relative">
                              <div className="flex items-center gap-2 mb-3">
                                <span className="inline-flex items-center gap-2 px-3 py-1 rounded-xl bg-blue-100/90 text-blue-950 text-[10.5px] font-bold tracking-wider uppercase shadow-2xs border border-blue-200/60">
                                  <span className="w-1.5 h-1.5 rounded-full bg-blue-600 animate-pulse" />
                                  Анализ ошибки
                                </span>
                              </div>
                              <div className="text-[14.5px] md:text-[15px] text-slate-800 leading-[1.8] font-normal">
                                <MathRenderer text={err.distractor_explanation_latex || err.distractor_explanation} className="text-[14.5px] md:text-[15px] text-slate-800 leading-[1.8]" />
                              </div>
                            </div>
                          )}
                          ...
                          {isDistractor && !err.distractor_explanation && (
                            <div className="rounded-2xl bg-blue-50/50 border border-blue-200/80 p-4.5 shadow-2xs">
                              <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-xl bg-blue-100/80 text-blue-950 text-[10.5px] font-bold tracking-wider uppercase border border-blue-200/60">
                                <span className="w-1.5 h-1.5 rounded-full bg-blue-600" />
                                Анализ ошибки
                              </span>
                              <p className="text-xs font-medium text-slate-700 mt-2">Ученик выбрал характерный ложный вариант ответа (дистрактор).</p>
                            </div>
                          )}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log("Successfully updated page.tsx with standard Tailwind grid classes, blue analysis block, and dot!");
} else {
