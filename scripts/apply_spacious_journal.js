                      ...
                      <div key={i} className="px-6 py-6 space-y-5 hover:bg-slate-50/30 transition-colors">
                        ...
                        <div className="rounded-2xl bg-slate-50/70 border border-slate-200/80 p-5 md:p-6 shadow-2xs">
                          ...
                          <MathRenderer
                            ...
                            block
                            className="text-[15.5px] md:text-[16px] text-slate-900 leading-[1.8] font-medium whitespace-pre-wrap break-words"
                          />
                        </div>
                          ...
                          {hasMcqOptions && (() => {
                            ...
                            return (
                              <div className="space-y-3">
                                <div className={\`grid gap-3.5 \${hasLongOptions ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2"}\`}>
                                  {uniqueOptions.map((item, oi) => {
                                    ...
                                    return (
                                      <div key={oi} className={\`flex items-start gap-3.5 p-4.5 rounded-2xl border transition-all \${
                                        isCorrect
                                        ...
                                        <span className={\`w-6.5 h-6.5 rounded-lg text-xs font-black flex items-center justify-center flex-shrink-0 mt-0.5 \${
                                        ...
                                        {isCorrect && <CheckCircle2 className="w-4.5 h-4.5 text-emerald-600 flex-shrink-0 mt-0.5" />}
                                        {isStudentAnswer && !isCorrect && <XCircle className="w-4.5 h-4.5 text-rose-400 flex-shrink-0 mt-0.5" />}
                                      </div>
                                    );
                                  })}
                                </div>
                                {!hasMatchedStudentOpt && err.student_answer && !["idk", "__wrong__", "test"].includes(err.student_answer.trim().toLowerCase()) && (
                                  <div className="flex items-center gap-2.5 px-4.5 py-3 rounded-xl bg-rose-50/70 border border-rose-200/80 text-sm font-medium text-rose-950">
                                    ...
                                    <MathRenderer text={err.student_answer_latex || err.student_answer} className="font-medium text-[15px]" />
                                  </div>
                                )}
                              </div>
                            );
                          ...
                          {!hasMcqOptions && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                              {/* Student answer */}
                              <div className="rounded-2xl border bg-white border-rose-200/90 p-4.5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2.5">
                                  <XCircle className="w-3.5 h-3.5 text-rose-400" />
                                  <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Ответ ученика</span>
                                </div>
                                {err.student_answer
                                  ? <MathRenderer text={err.student_answer_latex || err.student_answer} block className="text-[15px] md:text-[15.5px] font-medium text-rose-950 leading-[1.75] break-words" />
                                  : <span className="text-sm font-semibold text-slate-400">—</span>
                                }
                              </div>
                              {/* Correct answer */}
                              <div className="rounded-2xl border bg-emerald-50/70 border-emerald-200/90 p-4.5 shadow-2xs">
                                <div className="flex items-center gap-1.5 mb-2.5">
                                ...
                                {(err.correct_answer_latex || err.correct_answer)
                                  ? <MathRenderer text={err.correct_answer_latex || err.correct_answer || ""} block className="text-[15px] md:text-[15.5px] font-medium text-emerald-950 leading-[1.75] break-words" />
                                  : <span className="text-sm font-semibold text-slate-400">—</span>
                                }
                              </div>
                            </div>
                          ...
                          {isDistractor && err.distractor_explanation && (
                            <div className="rounded-2xl bg-gradient-to-br from-indigo-50/70 via-white to-indigo-50/30 border border-indigo-200/80 p-5 md:p-6 shadow-2xs relative">
                              <div className="flex items-center gap-2 mb-3">
                                <span className="inline-flex items-center px-3 py-1 rounded-xl bg-indigo-100/90 text-indigo-900 text-[11px] font-bold tracking-wide uppercase shadow-2xs">
                                  Анализ ошибки
                                </span>
                              </div>
                              <div className="text-[15px] md:text-[15.5px] text-slate-800 leading-[1.85] font-normal">
                                <MathRenderer text={err.distractor_explanation_latex || err.distractor_explanation} className="text-[15px] md:text-[15.5px] text-slate-800 leading-[1.85]" />
                              </div>
                            </div>
                          )}
                          ...
                          {isDistractor && !err.distractor_explanation && (
                            <div className="rounded-2xl bg-indigo-50/40 border border-indigo-100/80 p-4.5 shadow-2xs">
                              <span className="inline-flex items-center px-2.5 py-1 rounded-xl bg-indigo-100/70 text-indigo-900 text-[11px] font-bold tracking-wide uppercase">
                                Анализ ошибки
                              </span>
                              <p className="text-sm font-medium text-slate-700 mt-2">Ученик выбрал характерный ложный вариант ответа (дистрактор).</p>
                            </div>
                          )}
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log("Successfully updated page.tsx with spacious layout!");
} else {
